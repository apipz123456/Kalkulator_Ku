...
class Calculator:
    def __init__(self):
        ...
        self.window.title("MY Calculators @ojan90 ")
    ...
    def create_display_labels(self):
        total_label = tk.Label(self.display_frame, text=self.total_expression, anchor=tk.E , bg=LIGHT_GRAY,
                               fg= LABEL_COLOR, font=SMALL_FONT_STYLE)
    ...
    def create_digits_buttons(self):
        for digit,grid_value in self.digits.items():
            button = tk.Button(self.buttons_frame, text=str(digit), bg=WHITE, fg=LABEL_COLOR, font=DIGITS_FONT_STYLE,
                               borderwidth=0, command=lambda digit=digit: self.add_to_expression(digit))
    ...
    def create_decimal_button(self):
        button = tk.Button(self.buttons_frame, text='.', bg=WHITE, fg=LABEL_COLOR, font=DIGITS_FONT_STYLE,
                           borderwidth=0, command=lambda: self.add_to_expression('.'))

        button.grid(row=self.digits['.'][0], column=self.digits['.'][1], sticky=tk.NSEW)
        return button
